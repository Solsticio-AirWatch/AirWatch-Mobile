...
import { AirButton, AirInput, AirCard, SectionTitle, EmptyState, LoadingState, AirBadge } from '../components';
...
function SensorModal({ visible, initial, cities, onClose, onSave }) {
  ...
  React.useEffect(() => {
    if (visible) {
      setCityId(initial?.cityId ?? null); setName(initial?.name ?? '');
      setType(initial?.type ?? 'IoT'); setLocation(initial?.location ?? '');
      setSource(initial?.source ?? ''); setStatus(initial?.status ?? 'ACTIVE');
      setErrors({});
    }
  }, [visible]);
          ...
          <ScrollView>
            ...
            <View style={ms.chips}>
              {SENSOR_TYPES.map(t => (
                <Text key={t} onPress={() => setType(t)}
                  style={[ms.chipInline, type===t && ms.chipInlineActive]}>{t}</Text>
              ))}
            </View>
            ...
            <View style={ms.chips}>
              {SENSOR_STATUSES.map(st => (
                <Text key={st} onPress={() => setStatus(st)}
                  style={[ms.chipInline, status===st && ms.chipInlineActive]}>{st}</Text>
              ))}
            </View>
          </ScrollView>
...
export function SensorsScreen({ route }) {
  ...
  const statusColor = (s) => s==='ACTIVE' ? colors.primary : s==='MAINTENANCE' ? colors.warning : colors.danger;

  if (loading) return <LoadingState text="CARREGANDO SENSORES..." />;

  return (
    <View style={ss.container}>
      {cityFilter && <Text style={ss.filterLabel}>📍 Filtrando por: {cityFilter.name}</Text>}
      <FlatList data={sensors} keyExtractor={i => String(i.id)}
        contentContainerStyle={ss.list}
        ListEmptyComponent={<EmptyState icon="📡" title="NENHUM SENSOR" desc="Cadastre o primeiro sensor" />}
        renderItem={({ item }) => (
          <View style={ss.card}>
            ...
            <View style={ss.cardBody}>
              ...
              {item.location ? <Text style={ss.loc}>📍 {item.location}</Text> : null}
            </View>
            <View style={ss.actions}>
              <TouchableOpacity onPress={() => setModal({ visible:true, item })} style={ss.actionBtn}><Text>✏️</Text></TouchableOpacity>
              <TouchableOpacity onPress={() => handleDelete(item)} style={ss.actionBtn}><Text>🗑️</Text></TouchableOpacity>
            </View>
          </View>
        )}
      />
      ...
      <SensorModal visible={modal.visible} initial={modal.item} cities={cities}
...
const ss = StyleSheet.create({
  ...
  loc:         { color:colors.textMuted, fontSize:11, marginTop:2 },
