...
export default function HomeScreen({ navigation }) {
  ...
  const FEATURES = [
    { icon: 'satellite', color: colors.primary, title: 'Monitor',  desc: 'Qualidade do ar em tempo real por cidade',      screen: 'Dashboard' },
    canManage && { icon: 'map',     color: colors.accent,  title: 'Gestão',   desc: 'Gerencie países, cidades e sensores',           screen: 'GestaoTab' },
    canWrite  && { icon: 'bell',    color: colors.warning, title: 'Alertas',  desc: 'Configure e visualize alertas ambientais',      screen: 'AlertasTab' },
    canAdmin  && { icon: 'users',   color: '#BF5FFF',      title: 'Usuários', desc: 'Administre contas e permissões do sistema',     screen: 'AdminTab' },
  ].filter(Boolean);

  const handleNavigate = (screen) => {
    const tabs = ['GestaoTab', 'AlertasTab', 'AdminTab'];
    ...
    <ScrollView style={s.container} contentContainerStyle={s.content}>
      ...
      {FEATURES.map((f, i) => (
        <TouchableOpacity key={i} style={s.featureCard}
          onPress={() => handleNavigate(f.screen)} activeOpacity={0.8}>
